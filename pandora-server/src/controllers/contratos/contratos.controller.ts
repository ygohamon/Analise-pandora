...
import {
  procuraDadosContratosCNPJ,
  procuraDadosContratosCPF,
  procuraDadosContratosNuContrato,
  procuraDadosContratosNuLicitacao
} from './contratos.functions';
...
export let getContratosCNPJ = function (req: Request, res: Response){
  ...
  if(validaCNPJ(cnpj)){
    ...
    cf(log, procuraDadosContratosCNPJ, cnpj)
...
export let getContratosCPF = function (req: Request, res: Response){
  ...
  if(validaCPF(cpf)){
    ...
    cf(log, procuraDadosContratosCPF, cpf)
      .then(aditivos => res.status(200).send(aditivos))
      .catch(error => controllerError(res, error, nomeFuncao));
  } else {
      res.status(400).send(criaRespostaAPI(API_CODES.CODE_PARAM_INVALIDO, API_MSGS.MSG_PARAM_INVALIDO));
...
export let getContratosNuLicitacao = function (req: Request, res: Response){
...
export let getContratosNuContrato = function (req: Request, res: Response){

  const nuContrato = req.params.nucontrato;
  const processo = req.query.processo;
  ...
  if(cdUgestora && nuContrato){
    const valor = `${cdUgestora} - ${nuContrato}`;
    const log = new NovoLog({req, secao, item, chave: chaves.DADOSLICITACAO, valor, tipo: tipos_busca.DETALHADA, processo})

    cf(log, procuraDadosContratosNuContrato, nuContrato)
      .then(aditivos => res.status(200).send(aditivos))
      .catch(error => controllerError(res, error, nomeFuncao));
  } else {
    res.status(400).send(criaRespostaAPI(API_CODES.CODE_PARAM_INVALIDO, API_MSGS.MSG_PARAM_INVALIDO));
