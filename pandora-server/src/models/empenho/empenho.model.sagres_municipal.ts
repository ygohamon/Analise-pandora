...
const fonte   = MODEL_PRIORITY['tcepb.empenhos.municipal'].fonte;
const rank    = MODEL_PRIORITY['tcepb.empenhos.municipal'].rank;
const grupo   = MODEL_PRIORITY['tcepb.empenhos.municipal'].grupo;
...
export let getEmpenhosDetalhadoCPF_BD_SAGRES_SM = function (cpf: string){
  ...
  const query = () => {
    return db.query([
      ['CPF', ISql.Char(14), '000' + cpf],
        ...
        FROM ${modelConfig.get('SM_EMPENHOS')}
...
export let getEmpenhosDetalhadoCNPJ_BD_SAGRES_SM = function (cnpj: string){
...
export let getEmpenhoPagoDetalhadoCPF_BD_SAGRES_SM = function (cpf: string){
  ...
  const query = () => {
    return db.query([
      ['CPF', ISql.Char(14), '000' + cpf],
        ...
        FROM ${modelConfig.get('SM_EMPENHOS_PAGOS')}
        WHERE cpf_cnpj_credor=@CPF
        ORDER BY data_emissao DESC
      `);
  }
...
export let getEmpenhoPagoDetalhadoCNPJ_BD_SAGRES_SM = function (cnpj: string){
...
export let getEmpenhoPagoAnualizadoSimplificadoCPF_BD_SAGRES_SM = function (cpf: string){
...
export let getEmpenhoPagoAnualizadoSimplificadoCNPJ_BD_SAGRES_SM = function (cnpj: string){
