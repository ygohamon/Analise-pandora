...
@Component({
  selector: 'app-pessoa-mandado-dataview',
  templateUrl: './pessoa.mandado.dataview.component.html'
})
export class PessoaMandadoDataViewComponent implements OnInit {
  ...
  exportPdf() {
    ...
    const fn = (doc) => {
      doc.setFontSize(18);
      doc.text('Informações do Mandado de Prisão', 14, 22);
    }
    this.exporta.exportPdf(this.colunasTabela, this.mandado, filename, fn, 35);
  }

  exportExcel() {
    const filename = (this.exportFilename) ? `${this.exportFilename}.xlsx` : 'export.xlsx';
    this.exporta.exportExcel(this.mandado, filename);
  }
