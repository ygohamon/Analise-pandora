...
@Injectable()
export class PandoraTableService {
...
@Component({
  ...
  template: `
    <ng-template #captionpadrao>
      <div class="p-card-subtitulo my-2 text-left d-flex">
        {{caption}}

        <div class="export-button pointer text-primary ml-auto" (click)="exportCSV()">
          <span>CSV</span>
          <i class="pi pi-download"></i>
        </div>

        <div class="export-button pointer text-primary ml-2" (click)="exportPdf()">
          <span>PDF</span>
          <i class="pi pi-download"></i>
        </div>

        <div class="export-button pointer text-primary ml-2" (click)="exportExcel()">
          <span>Excel</span>
          <i class="pi pi-download"></i>
        </div>
      </div>

      <div class="text-left py-2" *ngIf="mostraFiltroGlobal">
        <i class="pi pi-search mr-2"></i>
        <input type="text" pInputText placeholder="Filtro Global" (input)="filterGlobal($event.target.value, 'contains')" style="width:90%">
      </div>
    </ng-template>

    <ng-template #cabecalhopadrao>
      <tr>
        <th *ngIf="mostraEspacamentoExpand" style="width: 3em"></th>
        <th *ngFor="let col of columns" [pandoraSortableColumn]="col.field">
            <ng-container *ngIf="!col.tooltip">{{col.header}}</ng-container>
            <ng-container *ngIf="!!col.tooltip">
              <span [pTooltip]="col.tooltip">{{col.header}}</span>
            </ng-container>

            <pandora-sortIcon [field]="col.field"></pandora-sortIcon>
        </th>
      </tr>

      <tr *ngIf="!!mostraFiltroColunas">
        <th *ngIf="mostraEspacamentoExpand" style="width: 3em"></th>
        <th *ngFor="let col of columns" [ngSwitch]="col.field">
            <input pInputText type="text" class="w-100 ui-inputtext" placeholder="Busca" (input)="filter($event.target.value, col.field, 'contains')">
        </th>
      </tr>
    </ng-template>

    <ng-template #sumariooadrao>
      {{utils.getFooterResultado(totalRecords)}}
    </ng-template>

    <ng-template #bodypadrao let-rowData let-expanded="expanded" let-columns="columns">
      <tr class="ui-selectable-row">

        <td *ngIf="mostraEspacamentoExpand">
        ...
        <td *ngFor="let col of columns">
            ...
            <span *ngIf="!dicionarioDados[col.field].fn && !dicionarioDados[col.field].fnRow">{{rowData[col.field]}}</span>
            <span *ngIf="!!dicionarioDados[col.field].fn">{{dicionarioDados[col.field].fn(rowData[col.field])}}</span>
            <span *ngIf="!!dicionarioDados[col.field].fnRow">{{dicionarioDados[col.field].fnRow(rowData)}}</span>
        </td>
      </tr>
    </ng-template>
      ...
      <div #container [ngStyle]="style" [class]="styleClass"
          ...
          <div *ngIf="!captionTemplate && !!caption" class="ui-table-caption ui-widget-header">
            <ng-container *ngTemplateOutlet="captionpadrao"></ng-container>
          </div>
          ...
          <div class="ui-table-wrapper" *ngIf="!scrollable">
              <table role="grid" #table [ngClass]="tableStyleClass" [ngStyle]="tableStyle">
                  ...
                  <thead class="ui-table-thead" *ngIf="headerTemplate">
                      <ng-container *ngTemplateOutlet="headerTemplate; context: {$implicit: columns}"></ng-container>
                  </thead>

                  <thead class="ui-table-thead" *ngIf="!headerTemplate">
                      <ng-container *ngTemplateOutlet="cabecalhopadrao; context: {$implicit: columns}"></ng-container>
                  </thead>

                  <tbody *ngIf="bodyTemplate" class="ui-table-tbody" [pandoraTableBody]="columns" [pTableBodyTemplate]="bodyTemplate"></tbody>
                  <tbody *ngIf="!bodyTemplate" class="ui-table-tbody" [pandoraTableBody]="columns" [pTableBodyTemplate]="bodypadrao"></tbody>
          ...
          <div class="ui-table-scrollable-wrapper" *ngIf="scrollable">
             <div class="ui-table-scrollable-view ui-table-frozen-view" *ngIf="frozenColumns||frozenBodyTemplate" [pandoraScrollableView]="frozenColumns" [frozen]="true" [ngStyle]="{width: frozenWidth}" [scrollHeight]="scrollHeight"></div>
             <div class="ui-table-scrollable-view" [pandoraScrollableView]="columns" [frozen]="false" [scrollHeight]="scrollHeight" [ngStyle]="{left: frozenWidth, width: 'calc(100% - '+frozenWidth+')'}"></div>
          </div>
          ...
          <p-paginator [rows]="rows" [first]="first" [totalRecords]="totalRecords" [pageLinkSize]="pageLinks" styleClass="ui-paginator-bottom" [alwaysShow]="alwaysShowPaginator"
              ...
              <!-- <div *ngIf="!summaryTemplate" class="ui-table-summary ui-widget-header">
                <ng-container *ngTemplateOutlet="sumariooadrao"></ng-container>
              </div> -->
  ...
  `,
  styleUrls: ['pandora.table.component.scss'],
  providers: [PandoraTableService, Table, TableService],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PandoraTable implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges {
  ...
  @Input() mostraFiltroGlobal: boolean = false;
  @Input() mostraFiltroColunas: boolean = false;
  @Input() mostraEspacamentoExpand: boolean = false;
  @Input() dicionarioDados = {};
  @Input() dicionarioDadosExpand = {};
  @Input() headerMP = {};
  @Input() orientacao;
  colunasDicionarioDados;
  colunasDicionarioDadosExpand;
  ...
  @ViewChild('resizeHelper', {static: true}) resizeHelperViewChild: ElementRef;

  @ViewChild('reorderIndicatorUp', {static: true}) reorderIndicatorUpViewChild: ElementRef;

  @ViewChild('reorderIndicatorDown', {static: true}) reorderIndicatorDownViewChild: ElementRef;

  @ViewChild('table', {static: true}) tableViewChild: ElementRef;
  ...
  constructor(public el: ElementRef, public zone: NgZone, public tableService: PandoraTableService, public cd: ChangeDetectorRef, public utils: UtilsService,
   ...
   */
  exportPdf() {
    const filename = (this.exportFilename) ? `${this.exportFilename}.pdf` : 'download.pdf';

    const fn = (doc) => {
      doc.setFontSize(18);
      doc.text(`${this.caption}`, 14, 22);

      // doc.setFontSize(11);
      // doc.setTextColor(100);
      // doc.text(this.descricao, 14, 30);
    }
    this.exporta.exportPdf(this.colunasDicionarioDados, this.value, filename, fn, 35);
  }

  exportExcel() {
    const filename = (this.exportFilename) ? `${this.exportFilename}.xlsx` : 'download.xlsx';
    this.exporta.exportExcel(this.value, filename);
  }
  ...
  ngOnInit() {
      ...
      this.colunasDicionarioDados = Object.keys(this.dicionarioDados).map(d => { return { field: d, header: this.dicionarioDados[d].nome, tooltip: this.dicionarioDados[d].dica }});
      this.colunasDicionarioDadosExpand = Object.keys(this.dicionarioDadosExpand);
      if (!!this.colunasDicionarioDados.length && !this.columns) {
        this.columns = this.colunasDicionarioDados;
      }
  ...
  public exportCSV(options?: any) {
      ...
      const colunasTabela = (!!this.dicionarioDados) ?
        Object.keys(this.dicionarioDados).map(d => { return { field: d, header: this.dicionarioDados[d].nome, tooltip: this.dicionarioDados[d].dica }}) :
        [];
      const colunasTabelaExpand = (!!this.dicionarioDadosExpand) ?
        Object.keys(this.dicionarioDadosExpand).map(d => { return { field: d, header: this.dicionarioDadosExpand[d].nome, tooltip: this.dicionarioDadosExpand[d].dica }}) :
        [];

      const colunasIdentificadas = [...new Set(Object.keys(this.dicionarioDados).concat(Object.keys(this.dicionarioDadosExpand)))];
      const todasColunas = Object.keys(this.value[0]);
      const colunasNaoIdentificadas = [];
      todasColunas.forEach(d => {
        if (!colunasIdentificadas.includes(d)) {
          colunasNaoIdentificadas.push({ field: d, header: d })
        }
      })

      const columns = (!this.dicionarioDados) ? this.columns : colunasTabela.concat(colunasTabelaExpand).concat(colunasNaoIdentificadas);
  ...
  onColumnDragEnter(event, dropHeader) {
      if (this.reorderableColumns && this.draggedColumn && dropHeader) {
          ...
          if (this.draggedColumn != dropHeader) {
              let dragIndex = DomHandler.indexWithinGroup(this.draggedColumn, 'pandoraReorderableColumn');
              let dropIndex = DomHandler.indexWithinGroup(dropHeader, 'pandoraReorderableColumn');
  ...
  onColumnDrop(event, dropColumn) {
      ...
      if (this.draggedColumn) {
          let dragIndex = DomHandler.indexWithinGroup(this.draggedColumn, 'pandoraReorderableColumn');
          let dropIndex = DomHandler.indexWithinGroup(dropColumn, 'pandoraReorderableColumn');
...
@Component({
  selector: '[pandoraTableBody]',
  template: `
    <ng-template #mensagemvaziapadrao>
      <tr>
        <td class="text-center font-weight-bold fs-3" *ngIf="!dt.mostraEspacamentoExpand" [attr.colspan]="columns.length">{{dt.utils.registroNaoEncontrado}}</td>
        <td class="text-center font-weight-bold fs-3" *ngIf="dt.mostraEspacamentoExpand" [attr.colspan]="columns.length+1">{{dt.utils.registroNaoEncontrado}}</td>
      </tr>
    </ng-template>

    <ng-template #expandpadrao let-rowData let-columns="columns">
        <tr>
          <td [attr.colspan]="columns.length + 1">
              <div class="ui-g ui-fluid">
                <div class="ui-g-12 p-1" *ngFor="let col of dt.colunasDicionarioDadosExpand">
                  <ng-container *ngIf="!dt.dicionarioDadosExpand[col].fn && !dt.dicionarioDadosExpand[col].fnRow">
                    <b>{{dt.dicionarioDadosExpand[col].nome}}:</b> {{rowData[col]}}
                  </ng-container>

                  <ng-container *ngIf="!!dt.dicionarioDadosExpand[col].fn">
                    <b>{{dt.dicionarioDadosExpand[col].nome}}:</b> {{dt.dicionarioDadosExpand[col].fn(rowData[col])}}
                  </ng-container>

                  <ng-container *ngIf="!!dt.dicionarioDadosExpand[col].fnRow">
                    <b>{{dt.dicionarioDadosExpand[col].nome}}:</b> {{dt.dicionarioDadosExpand[col].fnRow(rowData)}}
                  </ng-container>
                </div>
              </div>
          </td>
      ...
      <ng-container *ngIf="!dt.expandedRowTemplate && !dt.colunasDicionarioDadosExpand.length">
      ...
      <ng-container *ngIf="!dt.expandedRowTemplate && !!dt.colunasDicionarioDadosExpand.length">
          <ng-template ngFor let-rowData let-rowIndex="index" [ngForOf]="(dt.paginator && !dt.lazy) ? ((dt.filteredValue||dt.value) | slice:dt.first:(dt.first + dt.rows)) : (dt.filteredValue||dt.value)" [ngForTrackBy]="dt.rowTrackBy">
              <ng-container *ngTemplateOutlet="template; context: {$implicit: rowData, rowIndex: dt.paginator ? (dt.first + rowIndex) : rowIndex, columns: columns, expanded: dt.isRowExpanded(rowData), editing: (dt.editMode === 'row' && dt.isRowEditing(rowData))}"></ng-container>
              <ng-container *ngIf="dt.isRowExpanded(rowData)">
                  <ng-container *ngTemplateOutlet="expandpadrao; context: {$implicit: rowData, rowIndex: dt.paginator ? (dt.first + rowIndex) : rowIndex, columns: columns}"></ng-container>
              </ng-container>
          </ng-template>
      </ng-container>
      ...
      <ng-container *ngIf="dt.isEmpty() && !dt.loading && dt.emptyMessageTemplate">
          <ng-container *ngTemplateOutlet="dt.emptyMessageTemplate; context: {$implicit: columns, frozen: frozen}"></ng-container>
      </ng-container>

      <ng-container *ngIf="dt.isEmpty() && !dt.loading && !dt.emptyMessageTemplate">
          <ng-container *ngTemplateOutlet="mensagemvaziapadrao; context: {$implicit: columns, frozen: frozen}"></ng-container>
      </ng-container>
  `
})
export class PandoraTableBody {

  @Input("pandoraTableBody") columns: any[];
  @Input("pTableBodyTemplate") template: TemplateRef<any>;
  @Input() frozen: boolean;

  constructor(public dt: PandoraTable) {}
}

@Component({
  selector: '[pandoraScrollableView]',
  template: `
      ...
      <div #scrollBody class="ui-table-scrollable-body">
          <table #scrollTable [ngClass]="{'ui-table-scrollable-body-table': true, 'ui-table-virtual-table': dt.virtualScroll}" [class]="dt.tableStyleClass" [ngStyle]="dt.tableStyle">
              <ng-container *ngTemplateOutlet="frozen ? dt.frozenColGroupTemplate||dt.colGroupTemplate : dt.colGroupTemplate; context {$implicit: columns}"></ng-container>
              <tbody class="ui-table-tbody" [pandoraTableBody]="columns" [pTableBodyTemplate]="frozen ? dt.frozenBodyTemplate||dt.bodyTemplate : dt.bodyTemplate" [frozen]="frozen"></tbody>
          </table>
...
export class PandoraScrollableView implements AfterViewInit,OnDestroy,AfterViewChecked {

  @Input("pandoraScrollableView") columns: any[];
  ...
  @ViewChild('scrollableAligner', {static: true}) scrollableAlignerViewChild: ElementRef;
  ...
  constructor(public dt: PandoraTable, public el: ElementRef, public zone: NgZone) {
...
@Directive({
  selector: '[pandoraSortableColumn]',
  host: {
...
export class PandoraSortableColumn implements OnInit, OnDestroy {

  @Input("pandoraSortableColumn") field: string;
...
export class PandoraSortIcon implements OnInit, OnDestroy {
...
export class PandoraSelectableRow implements OnInit, OnDestroy {

  @Input("pandoraSelectableRow") data: any;
  ...
  constructor(public dt: PandoraTable, public tableService: PandoraTableService) {
      if (this.isEnabled()) {
...
@Directive({
  selector: '[pandoraSelectableRowDblClick]',
  host: {
      '[class.ui-selectable-row]': 'isEnabled()',
      '[class.ui-state-highlight]': 'selected'
  }
})
export class PandoraSelectableRowDblClick implements OnInit, OnDestroy {

  @Input("pandoraSelectableRowDblClick") data: any;
...
@Directive({
  selector: '[pandoraContextMenuRow]',
  host: {
      '[class.ui-contextmenu-selected]': 'selected',
      '[attr.tabindex]': 'isEnabled() ? 0 : undefined'
  }
})
export class PandoraContextMenuRow {

  @Input("pandoraContextMenuRow") data: any;
  ...
  constructor(public dt: PandoraTable, public tableService: PandoraTableService, private el: ElementRef) {
      if (this.isEnabled()) {
...
@Directive({
  selector: '[pandoraRowToggler]'
})
export class PandoraRowToggler {

  @Input('pandoraRowToggler') data: any;
...
@Directive({
  selector: '[pandoraResizableColumn]'
})
export class PandoraResizableColumn implements AfterViewInit, OnDestroy {
...
@Directive({
  selector: '[pandoraReorderableColumn]'
})
export class PandoraReorderableColumn implements AfterViewInit, OnDestroy {
...
@Directive({
  selector: '[pandoraEditableColumn]'
})
export class PandoraEditableColumn implements AfterViewInit {

  @Input("pandoraEditableColumn") data: any;
...
@Directive({
  selector: '[pandoraEditableRow]'
})
export class PandoraEditableRow {

  @Input("pandoraEditableRow") data: any;
...
@Directive({
  selector: '[pandoraInitEditableRow]'
})
export class PandoraInitEditableRow {

  constructor(public dt: PandoraTable, public editableRow: PandoraEditableRow) {}
...
@Directive({
  selector: '[pandoraSaveEditableRow]'
})
export class PandoraSaveEditableRow {
...
@Directive({
  selector: '[pandoraCancelEditableRow]'
})
export class PandoraCancelEditableRow {
...
export class PandoraCellEditor implements AfterContentInit {
  ...
  constructor(public dt: PandoraTable, @Optional() public editableColumn: PandoraEditableColumn, @Optional() public editableRow: PandoraEditableRow) { }
...
@Component({
  selector: 'pandora-tableRadioButton',
  template: `
...
export class PandoraTableRadioButton  {
...
@Component({
  selector: 'pandora-tableCheckbox',
  template: `
...
export class PandoraTableCheckbox  {
...
@Component({
  selector: 'pandora-tableHeaderCheckbox',
  template: `
      <div class="ui-chkbox ui-widget" (click)="onClick($event)">
          <div class="ui-helper-hidden-accessible">
...
export class PandoraTableHeaderCheckbox  {
...
@Directive({
  selector: '[pandoraReorderableRowHandle]'
})
export class PandoraReorderableRowHandle implements AfterViewInit {

  @Input("pandoraReorderableRowHandle") index: number;
...
@Directive({
  selector: '[pandoraReorderableRow]'
})
export class PandoraReorderableRow implements AfterViewInit {

  @Input("pandoraReorderableRow") index: number;
...
//   @Input('pandoraTooltip') set text(text: string) {
//       this._text = text;
//       if (this.active) {
//           if (this._text) {
...
@NgModule({
  imports: [CommonModule,PaginatorModule,TableModule, TooltipModule],
  // exports: [PandoraTable,SharedModule,PandoraSortableColumn,PandoraSelectableRow,PandoraRowToggler,PandoraContextMenuRow,PandoraResizableColumn,PandoraReorderableColumn,PandoraEditableColumn,PandoraCellEditor,PandoraSortIcon,PandoraTableRadioButton,PandoraTableCheckbox,PandoraTableHeaderCheckbox,PandoraReorderableRowHandle,PandoraReorderableRow,PandoraSelectableRowDblClick,PandoraEditableRow,PandoraInitEditableRow,PandoraSaveEditableRow,PandoraCancelEditableRow],
  exports: [PandoraTable, PandoraRowToggler, PandoraSelectableRow, PandoraSortableColumn],

  declarations: [PandoraTable,PandoraSortableColumn,PandoraSelectableRow,PandoraRowToggler,PandoraContextMenuRow,PandoraResizableColumn,PandoraReorderableColumn,PandoraEditableColumn,PandoraCellEditor,PandoraTableBody,PandoraScrollableView,PandoraSortIcon,PandoraTableRadioButton,PandoraTableCheckbox,PandoraTableHeaderCheckbox,PandoraReorderableRowHandle,PandoraReorderableRow,PandoraSelectableRowDblClick,PandoraEditableRow,PandoraInitEditableRow,PandoraSaveEditableRow,PandoraCancelEditableRow]
})
export class PandoraTableModule { }
