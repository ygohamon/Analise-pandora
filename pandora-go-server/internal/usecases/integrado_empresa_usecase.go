...
type IntegradoEmpresaUseCase struct {
	empresas repositories.IntegradoEmpresaRepository
}

// NewIntegradoEmpresaUseCase cria o orquestrador de empresa sem acoplar com pessoa.
func NewIntegradoEmpresaUseCase(empresas repositories.IntegradoEmpresaRepository) IntegradoEmpresaUseCase {
	return IntegradoEmpresaUseCase{empresas: empresas}
}
...
func (u IntegradoEmpresaUseCase) integradoExternoCNPJ(ctx context.Context, cnpj string, options types.SearchOptions) []map[string]any {
	...
	if rows, err := u.empresas.CortexExternoCNPJ(ctx, cnpj, options); err == nil {
	...
	if rows, err := u.empresas.CredlinkExternoCNPJ(ctx, cnpj); err == nil {
	...
	for _, task := range []struct {
		...
		if rows, err := task.run(ctx, cnpj); err == nil {
			...
			slog.WarnContext(ctx, "empresa external source failed", "source", task.name, "cnpj", mappers.MaskCNPJ(cnpj), "error", err)
		}
	}
	if options.JusBrasil {
		if rows, err := u.empresas.JusbrasilExternoCNPJ(ctx, cnpj); err == nil {
...
func (u IntegradoEmpresaUseCase) integradoCrawlersCNPJ(ctx context.Context, cnpj string, options types.SearchOptions) []map[string]any {
	started := time.Now()
	u.auditEmpresaSource(ctx, "empresa.crawlers", "crawlers", "integrado", cnpj, options.FontesAbertas, 0, 0, nil)
	// Crawlers sao um fluxo separado para preservar o contrato local/externo e facilitar timeouts por fonte aberta.
	out := []map[string]any{}
