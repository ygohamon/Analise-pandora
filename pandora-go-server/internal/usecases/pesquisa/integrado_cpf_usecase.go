...
type IntegradoCPFUseCase struct {
	pessoas integradoPessoaPorts
	crawler CrawlerSearchPort
}

// NewIntegradoCPFUseCase monta o orquestrador chamado por PessoaUseCase.
func NewIntegradoCPFUseCase(pessoas integradoPessoaPorts, crawler ...CrawlerSearchPort) IntegradoCPFUseCase {
	var crawlerPort CrawlerSearchPort
	if len(crawler) > 0 {
		crawlerPort = crawler[0]
	}
	return IntegradoCPFUseCase{pessoas: pessoas, crawler: crawlerPort}
}
