...
func (m SQLRepository) ExternoTransparenciaCPF(ctx context.Context, cpf string) ([]SourceResult, error) {
	...
	for _, month := range transparenciaCompetencias(m.modelVarInt("TRANSPARENCIA_CRAWLERS", "TRANSPARENCIA_MAX_MESES", 12)) {
		rows, err = client.GetArray(ctx, "api-de-dados/bolsa-familia-disponivel-por-cpf-ou-nis", map[string]string{"codigo": cpf, "anoMesCompetencia": month})
		add("transparencia.bolsa_familia."+month, "beneficio", mappers.TransparenciaBeneficioRows(rows, "bolsa_familia"), err)
	}
	for _, item := range []struct {
		name     string
		endpoint string
...
func transparenciaCompetencias(maxMonths int) []string {
	if maxMonths <= 0 {
		maxMonths = 12
	}
	now := time.Now()
	out := make([]string, 0, maxMonths)
	for i := 0; i < maxMonths; i++ {
		month := now.AddDate(0, -i, 0)
		out = append(out, fmt.Sprintf("%04d%02d", month.Year(), int(month.Month())))
	}
	return out
}
