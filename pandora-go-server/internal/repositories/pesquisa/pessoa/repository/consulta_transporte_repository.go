...
func (m SQLRepository) embarcacoesPorCampo(ctx context.Context, column string, value string, useLike bool) ([]map[string]any, error) {
	embarcacoes, ok := m.models.Table("BD_EMBARCACOES", "EMBARCACOES")
	if !ok || embarcacoes == "" || m.db == nil {
...
SELECT TOP 1000
	...
	` + sqlLiteral(m.modelVar("BD_EMBARCACOES", "FONTE_EMB", m.modelSigla("BD_EMBARCACOES", "EMB"))) + ` as fonte
FROM ` + embarcacoes + `
WHERE ` + column + ` ` + operator + ` @VALOR`
	return rowsToMaps(m.db.QueryContext(ctx, query, sql.Named("VALOR", argValue)))
}

func (m SQLRepository) aeronavesPorCondicao(ctx context.Context, where string, args ...any) ([]map[string]any, error) {
	rab, ok := m.models.Table("BD_RAB", "RAB")
	if !ok || rab == "" || m.db == nil {
		return []map[string]any{}, nil
	}
	query := `
SELECT TOP 1000
	CASE WHEN CPF_CNPJ <> '' THEN 'proprietario' WHEN CPF_CNPJ2 <> '' THEN 'operador' ELSE NULL END as tipo,
	...
	` + sqlLiteral(m.modelSigla("BD_RAB", "RAB")) + ` as fonte
FROM ` + rab + `
` + where
