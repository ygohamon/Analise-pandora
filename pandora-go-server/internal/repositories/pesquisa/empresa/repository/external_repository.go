...
// CortexEnderecoCNPJ consulta apenas o endereco Cortex da pessoa juridica para rota especifica.
func (m SQLRepository) CortexEnderecoCNPJ(ctx context.Context, cnpj string) ([]map[string]any, error) {
	model, ok := m.models.Get("WEBSERVICE_CORTEX")
	if !ok || !model.Ativado {
	...
	payload, err := cortexapi.NewClient(model).Get(ctx, basePessoas+"/pessoajuridica/"+cnpj, "")
	if err != nil || payload == nil {
		return []map[string]any{}, err
	}
	return mappers.CortexEmpresaEnderecoRows(mappers.AsMap(payload), fonte), nil
}

// CortexEmbarcacaoCNPJ atende a rota especifica de embarcacoes por CNPJ.
func (m SQLRepository) CortexEmbarcacaoCNPJ(ctx context.Context, cnpj string) ([]map[string]any, error) {
	model, ok := m.models.Get("WEBSERVICE_CORTEX")
	if !ok || !model.Ativado {
		return []map[string]any{}, nil
	}
	...
	return cortexEmpresaEmbarcacaoRows(ctx, cortexapi.NewClient(model), baseVeiculos, cnpj, "", fonte)
}
