...
// UpdatePassword e chamado pelo fluxo de perfil/troca de senha e grava a senha nova do usuario.
func (r SQLUsuarioRepository) UpdatePassword(ctx context.Context, id int64, password string) error {
	table, ok := r.models.Table("BD_PANDORA", "USUARIO")
	if !ok || r.db == nil {
		return types.ErrModelNotConfigured
	}
	_, err := r.db.ExecContext(ctx, `
IF COL_LENGTH('`+table+`', 'senha_anterior') IS NULL
BEGIN
	UPDATE `+table+`
	SET senha = @SENHA, proximo_logon = 0, validado = 1, data_atualizacao = GETDATE()
	WHERE id = @ID
END
ELSE
BEGIN
	UPDATE `+table+`
	SET senha_anterior = senha, senha = @SENHA, proximo_logon = 0, validado = 1, data_atualizacao = GETDATE()
	WHERE id = @ID
END`,
		sql.Named("ID", id),
...
// ResetTFA e chamado pelo administrador para limpar a configuracao de duplo fator.
func (r SQLUsuarioRepository) ResetTFA(ctx context.Context, login string) error {
	table, ok := r.models.Table("BD_PANDORA", "USUARIO")
	if !ok || r.db == nil {
		return types.ErrModelNotConfigured
	}
	_, err := r.db.ExecContext(ctx, `
UPDATE `+table+`
SET setupTFA = 0, keyTFA = NULL, data_atualizacao = GETDATE()
WHERE LOWER(LTRIM(RTRIM(login))) = @LOGIN`,
		sql.Named("LOGIN", normalizeLogin(login)),
	)
	if err != nil {
