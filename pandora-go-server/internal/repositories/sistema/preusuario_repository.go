...
// UpdateUserPasswordReset redefine a senha local e força troca no proximo login.
func (r SQLUsuarioRepository) UpdateUserPasswordReset(ctx context.Context, id int64, password string) error {
	table, ok := r.models.Table("BD_PANDORA", "USUARIO")
	if !ok || r.db == nil {
		return types.ErrModelNotConfigured
	}
	query := `
DECLARE @TBL NVARCHAR(256) = N'` + table + `';
DECLARE @EXISTE_COLUNA BIT = CASE WHEN EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID(@TBL) AND name = 'senha_anterior') THEN 1 ELSE 0 END;
IF (@EXISTE_COLUNA = 0)
BEGIN
	UPDATE ` + table + ` SET senha=@SENHA, proximo_logon=1, validado=0 WHERE id=@ID
END
ELSE
BEGIN
	UPDATE ` + table + ` SET senha_anterior=senha, senha=@SENHA, proximo_logon=1, validado=0 WHERE id=@ID
END`
	_, err := r.db.ExecContext(ctx, query, sql.Named("ID", id), sql.Named("SENHA", utils.HashSenha(password)))
	return dbExecErr(err)
}

// UpdatePreUserTerm grava o termo enviado pelo dialog Meu Perfil na pessoa vinculada.
func (r SQLUsuarioRepository) UpdatePreUserTerm(ctx context.Context, idPessoa int64, termoBase64 string) error {
	table, ok := r.models.Table("BD_PANDORA", "PESSOA")
	if !ok || r.db == nil {
		return types.ErrModelNotConfigured
	}
	_, err := r.db.ExecContext(ctx, `
UPDATE `+table+`
SET termo = @TERMO, data_atualizacao = GETDATE()
WHERE id = @ID`,
		sql.Named("ID", idPessoa),
		sql.Named("TERMO", termoBase64),
	)
	if err != nil {
