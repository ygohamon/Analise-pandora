...
func (r SQLUsuarioRepository) find(ctx context.Context, login string, id int64) (types.Usuario, error) {
	...
	if err := r.ensureUserPreferenceColumns(ctx, usuario); err != nil {
		return types.Usuario{}, err
	}
...
SELECT TOP 1
	...
	ISNULL(U.tema_escuro, 0) as temaEscuro,
	ISNULL(U.espa, 0) as espa
FROM `+usuario+` U
	INNER JOIN `+perfil+` P ON (U.id_perfil = P.id)
...
WHERE U.id=@ID OR LOWER(LTRIM(RTRIM(U.login)))=@LOGIN`,
	...
	var pep, rif, jusbrasil, cortex, seeu, transparencia, fontesAbertas, temaEscuro, espa bool
	if err := row.Scan(
		&out.ID,
		&idPessoa,
		&idAtivador,
...
func (r SQLUsuarioRepository) ensureUserPreferenceColumns(ctx context.Context, table string) error {
	columns := []struct {
		name       string
		definition string
	}{
		{"tema_escuro", "BIT NOT NULL CONSTRAINT DF_USUARIO_TEMA_ESCURO DEFAULT (0)"},
		{"espa", "BIT NOT NULL CONSTRAINT DF_USUARIO_ESPA DEFAULT (0)"},
	}
