...
func (h Handler) resumoAPIs(w http.ResponseWriter, r *http.Request) {
	year, err := shared.OptionalQueryInt(r, "ano")
	if err != nil {
		httpx.ErrorFrom(w, err)
		return
	}
	month, err := shared.OptionalQueryInt(r, "mes")
	if err != nil {
		httpx.ErrorFrom(w, err)
		return
	}
...
func (h Handler) alertasAuditoria(w http.ResponseWriter, r *http.Request) {
	minimum := 2
	if parsed, err := shared.OptionalQueryInt(r, "minimo"); err != nil {
		httpx.ErrorFrom(w, err)
		return
	} else if parsed != nil {
		minimum = *parsed
...
func (h Handler) logsUsuarioQuery(w http.ResponseWriter, r *http.Request) {
	top, err := shared.OptionalQueryInt(r, "top")
	if err != nil {
		httpx.ErrorFrom(w, err)
		return
	}
