...
func (h Handler) mailInfo(w http.ResponseWriter, _ *http.Request) {
	...
	data := map[string]any{
		...
		"method":    firstNonEmpty(service, host, "smtp"),
...
func (h Handler) enviarEmail(w http.ResponseWriter, r *http.Request) {
	...
	recipients := emailRecipients(payload.Email.To)
	if payload.Email.All || hasBroadcastRecipient(payload.Email.To) {
		users, err := h.auth.ActiveMailerUsers(r.Context())
		if err != nil {
			httpx.ErrorFrom(w, err)
			return
		}
		recipients = append(recipients, emailRecipients(users)...)
	}
	...
	from := firstNonEmpty(os.Getenv("MAIL_FROM"), os.Getenv("MAIL_USER"), "pandora@mpsp.mp.br")
	...
	body := buildMailMessage(from, recipients, payload.Email.Subject, payload.Email.Content)
